...
2. Note, the Hosting Bundle has an issue with the ASP.NET Core module. If you would like to install the ASP.NET Core Module, please install the 3.0 Preview 2 [Hosting Bundle][dotnet-hosting-win.exe] and run on a command line with the paramaters `OPT_NO_SHAREDFX=1 OPT_NO_RUNTIME=1`. Then install the appropriate .NET and ASP.NET runtimes.
